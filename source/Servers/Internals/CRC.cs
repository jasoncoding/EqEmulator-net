...
namespace EQEmulator.Servers.Internals
{
    internal static class CRC
    {
        ...
        internal static uint ComputeChecksum(byte[] buffer, int length, uint key)
        {
            uint nCrc = _CrcTable[(~key) & 0xFF];
            nCrc ^= 0x00FFFFFF;
            uint nIndex = (key >> 8) ^ nCrc;
            nCrc = (nCrc >> 8) & 0x00FFFFFF;
            nCrc ^= _CrcTable[nIndex & 0xFF];
            nIndex = (key >> 16) ^ nCrc;
            nCrc = (nCrc >> 8) & 0x00FFFFFF;
            nCrc ^= _CrcTable[nIndex & 0xFF];
            nIndex = (key >> 24) ^ nCrc;
        ...
        internal static uint ComputeChecksum(byte[] buffer)
        {
            uint data;
            uint check = 0xffffffff;

            if (buffer.Length < 4)
                throw new ArgumentException("buffer to compute checksum for is too small", "buffer");
